...
def write_track_to_vtk(track_points, filename):
	"""
	Write a list of [x, y, z] points to an ASCII VTK PolyData file.
	"""
	npts = len(track_points)

	with open(filename, "w") as f:
		# Header
		f.write("# vtk DataFile Version 4.2\n")
		f.write("Hurricane eye track\n")
		f.write("ASCII\n")
		...
		if npts > 1:
			# "LINES nlines total_num_indices"
			# total_num_indices = npts + number_of_lines
			f.write(f"LINES 1 {npts + 1}\n")
			f.write(f"{npts} " + " ".join(str(i) for i in range(npts)) + "\n")

	print(f"Wrote {npts} track points to {filename}")


def hurricane_eye_tracker(x_grid, y_grid, lambert_conformal,
						  uvel_erf, vvel_erf, init_lon, init_lat, forecast_hour, track_points):

	print("init_lat is ", init_lat)
	print("init_lon is ", init_lon)

	transformer = Transformer.from_crs("EPSG:4326", lambert_conformal, always_xy=True)
	inverse_transformer = Transformer.from_crs(lambert_conformal, "EPSG:4326", always_xy=True)

	init_x, init_y = transformer.transform(init_lon, init_lat)

	x_grid = np.asarray(x_grid)
	y_grid = np.asarray(y_grid)

	filename = "Output/hurricane_track_latlon.txt"
	vtk_filename = f"Output/track_latlon/hurricane_eye_track_xy_{forecast_hour}.vtk"
	...
	if file_empty:
		# --- CASE 1: File empty → find grid point closest to init_x, init_y ---
		dist2 = (x_grid - init_x)**2 + (y_grid - init_y)**2
		j_min, i_min = np.unravel_index(np.argmin(dist2), dist2.shape)

		print("j_min, i_min are", j_min, i_min)
	
		   # Get lat, lon of closest point
		lon_pt, lat_pt = inverse_transformer.transform(x_grid[j_min, i_min], y_grid[j_min, i_min])
	  
		track_x, track_y = transformer.transform(lon_pt, lat_pt)
		track_points.append([track_x, track_y, 10e3])
		write_track_to_vtk(track_points, vtk_filename)
		...
		# --- CASE 2: File non-empty → use latest entry and find min-velocity point ---
		with open(filename, "r") as f:
			lines = f.readlines()
			_, last_lon, last_lat = map(float, lines[-1].split())

		# Convert last lat/lon to projected x,y
		last_x, last_y = transformer.transform(last_lon, last_lat)

		# Radius (200 km)
		radius = 200.0 * 1000.0  # meters
		dist2 = (x_grid - last_x)**2 + (y_grid - last_y)**2

		# Mask points outside 200 km
		mask = dist2 <= radius**2
		if not np.any(mask):
			print("Warning: No grid points found within 200 km radius.")
			return

		# --- Use velocity at k=1 only ---
		k = 0
		...
		# Apply mask
		vel_masked = np.where(mask, vel_mag, np.inf)

		# Find point with minimum velocity magnitude
		j_min, i_min = np.unravel_index(np.argmin(vel_masked), vel_masked.shape)
...
def write_binary_vtk_cartesian(date_time_forecast_str, output_binary, domain_lats, domain_lons,
							   ...
							   k_to_delete, lambert_conformal, forecast_hour, point_data=None, init_lon=-1e3, init_lat=-1e3, track_points=None):
